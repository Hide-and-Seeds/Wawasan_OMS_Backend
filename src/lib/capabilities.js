...
const CAPABILITIES = [
  ...
  // Clearing the board is NOT a capability. It shipped as one on 2026-09-24, defaulting
  // to nobody — but a default of nobody and a switch that can be flipped are different
  // things, and this is the one action in the app that changes what every other person
  // sees the next morning. Leaving it grantable meant it could be handed to a role by a
  // tired click in the panel. It is hardcoded to super_admin in routes/orders.js now,
  // for the same reason the permissions panel itself is: the biggest levers must not be
  // reachable through the screen that hands out levers.

...
-- ─────────────────────────────────────────
-- OUTBOUND MESSAGE QUEUE (WhatsApp auto-messages + morning brief)
-- ─────────────────────────────────────────
create table if not exists message_queue (
  id         uuid primary key default gen_random_uuid(),
  channel    text not null default 'whatsapp',
  recipient  text not null,                       -- MSISDN (e.g. 60123456789) or 'admin'
  body       text not null,
  order_id   uuid references orders(id) on delete set null,
  kind       text not null,                       -- received | out_for_delivery | delivered | morning_brief | test
  status     text not null default 'queued' check (status in ('queued','sending','sent','failed','cancelled')),
  attempts   int  not null default 0,
  provider   text,                                -- 'log' | 'wwebjs'
  error      text,
  created_at timestamptz not null default now(),
  sent_at    timestamptz
);
-- idempotent enqueue per order+kind (NULL order_id rows — e.g. briefs — never collide)
